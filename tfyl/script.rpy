...
label tfyl_gates:
    ...
    "Потресканый асфальт давно не видал резины автомобилей, что ж говорить о ноге человека."
    ...
    "Стены исписаны, бардюров не хватает, а асфальт: местами вырванный, местами провалившейся.{w} Казалось, прошло куда больше времени, чем есть на самом деле."
    "Пеший маршрут подходил к концу.{w} Издали показались парковка и ворота лагеря."
    "Сколько я не бывал, но именно этот вид встречал пионеров, счастливых из-за конца длительного маршрута."
    "Последовательно счастье перерастало в нечто большее.{w} Юнцы начинали беситься, а кто по старше - активно обсуждать планы.{w} В те мгновения салон преображался."
    "Как помню: В первую смену я прижался к вожатой и без конца задавал вопросы.{w} Только что именно?{w} Вряд ли скажу.{w} Хоть убей."
...
label .search:
    ...
    menu (screen="tfyl_choice"):
        ...
        "me" ((.11, .32)) if places[:-1] and not places[3]:
            ...
            "Пока Славяна и Лена бросились успокаивать Мику, я собрался с силами и присоеденился к ним."
...
label .search:
    ...
    menu (screen="tfyl_choice"):
        ...
        "?" ((.05, .7)) if not places[1]:
            ...
            "Живой уголок располагался прямо за зданием и собирал в себе дюжиную коллекцию."
        ...
        "?" ((.45, .5)) if not places[2]:
            ...
            "Обойдя здание по кругу я сделал вывод, что все возможное вынесено, а невозможное побито и оторвано.{w} Это же касалось помещений."
...
label tfyl_musclub:
    ...
    "Да, заросло, но дух того времени все равно отличался.{w} Даже если закрыть глаза, то можно вслушаться, прочуствовать."
    window hide dissolve
...
label .search:
    ...
    menu (screen="tfyl_choice"):
        ...
        "?" ((.18, .6)) if not places[0]:
            ...
            "Все великолепие таилось в цветение."
        ...
        "me" ((.92, .57)) if places[:2] and not places[3]:
            ...
            "Я постыдился своего вида.{w} Постыдился, что отнимаю время у такой прелестной.{w} Но все равное мое мнение не изменится, хоть под ружье ставь."
...
label .search:
    ...
    menu (screen="tfyl_choice"):
        ...
        "?" ((.18, .39)) if not places[0]:
            ...
            "Девушка, та самая с зелёной косой, была довольно скрьезной.{w} Рядом с ней находилась девчушка чуть младше, её отличали каштановые волосы и брошь."
            "Находились обе у заброшенного лагеря.{w} Одна перерезала проволку, пока другая внимательно наблюдала.{w} Значит, всё-таки без меня смогли пробраться."
        ...
        "me" ((.13, .25)) if places[:4] and not places[4]:
            ...
            "Подруга облокотилась о соседнию колонну.{w} Её поведение меня удивляло."
            ...
            with dissolve
            "Она повернулась, сделав непонимающие лицо.{w} Будто сама невинность, покачала головой."
            ...
            "Птичьи трели были усладой для ушей.{w} Они раслабляли под бесконечными тенями, скрывавших от палящего солнца."
        ...
        "me" ((.57, .74)) if places[:4] and not places[5]:
            ...
            "Однако эти слова вызвали в Ульянка бурю негатива.{w} Она воскликнула и обратилась к Мику."
            ...
            "Она, как и я, понимала, что такое количество для меня нереальное особенно с Ульяной на спине."
...
""", remove=True)
            ...
            "Дрож прошла по телу.{w} Я быстро схватил письмо и прыгнул к выходу."
...
label tfyl_polyana:
    ...
    show screen tfyl_backdrop(3, "Бесконечная игра", "images/bg/ext_polyana_sunset.jpg", im.Composite((900,1080), (0,0), "images/sprites/normal/un/un_1_body.png",(0,0), "images/sprites/normal/un/un_1_pioneer.png",(0,0), "images/sprites/normal/un/un_1_smile.png") ) with fade
...
label .search:
    ...
    menu (screen="tfyl_choice"):
# это  флэш идет после .yama по идее тогд
        "me" ((.02, .38)) if places[:3] not places[3]:
            $ places.set_true(3)
...
label tfyl_convert:
    ...
    hide tfyl_convert
    with dissolve
    $ mail_txt_print("Спустя два года лагерь позволил себя увидеть.{w} Все это время я откладывала поездку и поняла, что зря…{w}\nЛюди построили.{w} Они же и разобрали.{w} От лагеря остались одни воспоминания и холодные здания.{w} Но я видела нечто больше…{w}\nВторую жизнь.{w} Тут все можно было восстановить, да и АЭС, которая неподалеку, перестала функционировать.{w} Вот она возможность!{w}\nЖаль, но помощи я не дождалась.{w} Все мои рассказы вызвали один вопрос: Для чего?{w}\nРаскрывать прелесть лагеря я не стала.{w} Не поймут.{w} Они ведь тут не жили.{w} Да и то, что я видела, посчитают бредом.")
    call screen tfyl_mail
    $ mail_txt_print("Поэтому начала одна.{w} Восстановление заняло больше полутора года, если не считать зиму.{w} В эту пору я уезжала и оставляла заместо себя друга\nОн топил здания через кочегарку и жил тут.{w} Весной я сменяла его и так по кругу.{w} Пока не случилось это…{w}\nОбход лагеря завершался ночью, потом отбой на восемь часов с последующим подъемом.{w} За это время случился налет, видимо, они следили за моим распорядком\nПроснулась.{w} Увидела разрушения и просто отчаялась.{w} Они вынесли все, а то что не смогли попросту сломали!{w}\nПроснулась.{w} Увидела разрушения и просто отчаялась.{w} Они вынесли все, а то что не смогли попросту сломали!{w}\n")
    call screen tfyl_mail
    ...
    scene  bg tfyl_polyana at beg_25
    $ renpy.pause(3,hard=True)
    ...
    play music evnening_25 fadein 2 # да, я уже подготовил
