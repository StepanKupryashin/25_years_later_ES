...
label .loads: # Все необходимое для третей главы

    call tfyl_chapter_2.loads
...
label tfyl_polyana:
    ...
    'Тихое место, скрывавшее под собой кучу развлечений.' # П.М # Я все еще не могу понять почему banderak использует ' вместо "
...
label .search:
    ...
    menu(screen="tfyl_choice"):
        "me" ((.02, .38)) if places[:3] and  not places[3]:
            ...
            'Я отошёл и присел на поваленное бревно.{w} Товарищи сразу обратили внимание, особенно Алиса.{w} Её состояние чем-то напоминало моё.'
            ...
            play sound sfx_punch_medium
            extend ' Из-за неё я ни черта не видел!' with vpunch
            'Накал страстей оборвала Алиса, которая громко воскликнула.{w} Её раздражение пропало и вернулась та самая, которая всем нравится.'
        ...
        "me" ((.4, .2)) if places[:3]  and not places[5]:
            ...
            "Откуда она взяла все это - другой вопрос.{w} Больше всего меня привлекли движения её рук, настолько плавно и четко, будто рисовала шедевр."
            ...
            "Откуда он у неё взялся там - неизвестно, да и не имеет значения."
            ...
            "Лена закончила, встала, отошла.{w} На месте её работы оказался красивый бант, который стягивал два конца."
...
"Проснулась. Увидела разрушения и просто отчаялась. Они вынесли все, а то что не смогли попросту сломали!\nОпять пришлось начинать все сначала...")
    ...
    "Я ощущал её эмоции в тот день.{w} Злобу на преступников, а потом на себя, что не уследил.{w} Затем пустоту.{w} Все начинать с нуля…"
...
#понял
    ...
    # дада
        ...
        '?' ((0.28, 0.47)) if not places[1]:
            ...
            'Да.{w} Эта девчонка занималась вместе со мной, хотела спарринг.{w} Правда, сразу перехотела, когда её оппонентом стала кукла.'
        ...
        '?' ((0.54, 0.28)) if not places[2]:
            ...
            'Тоже отказали.{w} На третий раз мы решили привести вожатую, её-то слова наверняка послушают{w}, пока не узнали кое-что важное.'
        ...
        'me' ((0.44,0.85)) if places[:3] and not places[3]:
            ...
            'Ульяна обязательно придет переодеться в пионерскую форму.{w} Нет.{w} Воровать её я не собирался.{w} Тут было кое-что получше.'
