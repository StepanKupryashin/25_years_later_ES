...
label .search:
    ...
    menu (screen="tfyl_choice"):
        ...
        "me" ((.11, .32)) if places[:-1] and not places[3]:
            ...
            "Лену я больше не видел.{w} Письма от неё я не получал, а отправленные лично мной постоянно возвращались."
...
label tfyl_clubs:
    ...
    'Бесконечный просмотр "Джейсона Вурхиза" каждый вечер!{w} Других кассет к сожалению не появлялось.'
