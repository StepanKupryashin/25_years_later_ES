...
label .yama:
    ...
    'За свое существование она собрала немало слухов.{w} И если отсеивать более бредовые и сказочные,{w} то нарисовывалась легенда.'
...
label .flashback:
    ...
    'Пример - топь, занимавшая половину всей площади. Человеческим ограждениями внимания уделялось мало. Их прохождение занимало не более пяти минут.'
    'Остальные пунктиры я не смог разобрать. Расшифровку в правом нижнем углу загораживал Шурик.'
    ...
    'К Алисе прильнула кровь и её глаза распахнулись горячим пламенем.' 
    ...
    'Но была загвоздка.{w} Все это необходимо забрать и, после экспедиции, вернуть.{w} Причем нельзя стать замеченным, иначе все, дыры подладают, и вынос станет не возможным.'
...
label .base:
    ...
    'Этакое окончание появилось не давно.{w} Все из-за отбитых пальцев, заноз, ранок.{w} Вожатые устали не обращать внимание и приняли меры.'
...
label .flashback_2:
    'Благодаря маленькой договоренности, повара варили нам освежающий напиток.{w} Летний. Ягодный.{w} Морс.'
...
label .bonfire:
    ...
    'Нет лучше еды с открытого огня и песен от тепла.{w} Особенно в сумерки{w}, когда вокруг пугающая темнота, пронзжающая самых отважных.'
...
label  .pond:
    ...
    show window
    with flash 
    '... Уже с полчаса меня вели с черной повязкой. Глаза бегали туда сюда, искали просвет.{w} Безуспешно.'
    ...
    hide un with dissolve
    'Она заметалась, но быстро сорентировалась.{w} Достала бутылку и быстро опрокинула на мои ноги.'
    ...
    me 'Это просто...{w} Не повезло.{w} Я ж мастер заныривать!'
...
label .convert:
    ...
    #тут должен быть конверт(письмо или записка(как удобно))
